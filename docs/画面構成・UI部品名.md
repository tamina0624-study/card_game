...
- 作成時点(2026-07-17)は `PHP_BRIDGE_URL`(本番MySQL)が本セッションから疎通不可だったため、キャラクター一覧・デッキ一覧・対戦・ストーリーなど**データ取得を伴う画面は実データ入りのキャプチャが撮れていない**。該当画面は空データ/エラー表示のキャプチャ+実装コードの読み取りで領域構成を復元している。ブリッジ復旧後に実データ入りで撮り直すとより分かりやすくなる。
