...
/**
 * ストーリービートのタイトル・あらすじ(大枠)・プレイヤー名からユーザーメッセージを
 * 構築する。このモジュール自身はAI呼び出しを行わない(`lib/stories/generate.ts`が担当)。
 */
export function buildStoryPrompt(beatTitle: string, outline: string, username: string): string {
  return [
    `プレイヤー名: ${username}`,
    `タイトル: ${beatTitle}`,
    `あらすじ(大枠):`,
